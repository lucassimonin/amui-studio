...
Je suis une **agence web**. Ce dépôt est le **site amuï studio**, généré depuis mon CMS starter kit Symfony (projet `starter-kit`). Même stack, mêmes conventions. Le framework a été **mis à niveau sur la dernière version du kit** (actualités, révisions, mode maintenance, SEO checker, sélecteur de médias, éditeur riche, multilingue, config de déploiement FrankenPHP) tout en conservant le contenu et le design propres à amuï.
...
Homepage = intégration de la maquette **amuï studio** (portfolio one-page minimaliste, palette ink/paper/accent, fonts Lexend + Inter) : `hero_split`, `project_grid` (4 projets), `split_intro` (L'Agence), `contact_simple`. Deuxième page : mentions légales (rich_text, noindex). Ces blocs et ce thème sont **propres à amuï** (dans `src/Block/Type/`, `templates/blocks/`, `assets/styles/app.css`, `src/DataFixtures/AppFixtures.php`).

Le reste (entités, contrôleurs, admin, services, config, déploiement) provient du **framework du kit** et ne doit pas être personnalisé sans raison — pour repartir d'une base propre, resynchroniser depuis `starter-kit`.

> Mise à niveau effectuée depuis une version ancienne du kit. Les gabarits communs ajoutés (actualités, maintenance) sont reskinés via les tokens `--color-sand/forest/sage/…` ajoutés dans `app.css` (mappés sur la palette amuï). `app.locales` vaut `['fr','en']` mais amuï n'a pas de contenu `en` : réduire à `['fr']` dans `config/services.yaml` si le multilingue n'est pas voulu.
